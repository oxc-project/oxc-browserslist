...
pub fn save_bin_compressed(file: &str, bytes: &[u8]) {
    ...
    // standard raw-deflate stream, so the runtime decoder (the hand-rolled inflate in
    // `src/data/caniuse/compression.rs`) reads it unchanged and no consumer-facing dependency changes.
